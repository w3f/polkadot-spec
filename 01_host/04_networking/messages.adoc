...
response (<<defn-msg-state-response>>) or a warp sync response (<<>>).
====
...
the state request (<<defn-msg-state-request>>) on the `x` substream and contains
accumulated proof of multiple authority set changes
...
indicates that the warp sync has been completed.
====
