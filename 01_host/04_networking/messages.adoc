...
.<<defn-wrap-sync-request, Warp Sync Request>>
====
A **warp sync request** is sent to a node to request authority set changes. The
message is a single 32-byte Blake2 hash which indicates the block from which the
sync should start.
====

[#defn-wrap-sync-response]
.<<defn-wrap-sync-response, State Response>>
====
The state response contains a list of key/value entries with an associated proof.
...
where _KeyValuesStateEntry_ is of the following format:
