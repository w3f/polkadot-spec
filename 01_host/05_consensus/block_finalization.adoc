...
* Only for round stem:[r] for which the node stem:[n] has invoked Algorithm <<algo-grandpa-round>> and round stem:[r+1] if latexmath:[$n$] is a GRANDPA voter and has already caught up to its peers according to the process described in Section <<sect-grandpa-catchup>>.
