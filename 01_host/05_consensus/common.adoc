...
* _2_ implies *forced change*: Schedule a forced authority set change after the
given delay of stem:[N_("delay") := m + |"SubChain"(B,B')|] where stem:[B'] is
the block after stem:[m] where the change is applied. The earliest digest
of this type in a block will be respected.
+
Forced changes are explained further in <<sect-finality-forced-changes>>.
* _3_ implies *on disabled*: An index to the individual authority in the current
