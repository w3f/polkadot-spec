...
given delay of stem:[N_("delay") := |"SubChain"(B,B')|] where stem:[B'] is a the
block where the changes into effect. The earliest digest of this type in a
single block will be respected, unless a force change is present, in which case
the force change takes precedence.
* _2_ implies *forced change*: Schedule a force authority set change after the
given delay of stem:[N_("delay") := m + |"SubChain"(B,B')|] where stem:[B'] is a
the block after stem:[m] where the changes go into effect. The earliest digest
of this type in a single block will be respected.
+
