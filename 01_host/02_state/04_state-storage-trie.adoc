...
system storage for the Runtime (<<sect-entrypoints-into-runtime>>). Here we
...
needs to be re-arranged and hashed in a _radix tree_, which
...
stem:[N_(n+1)] should not be added to the length. The hashed subvalue for
variants stem:[001] and stem:[0001] is described in <<defn-hashed-subvalue>>.
...
== Managing Multiple Variants of State
