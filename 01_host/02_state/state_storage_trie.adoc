...
====
The *node header* specifies the node variant and the size of the partial key in
nibbles. Both pieces of information are indicated in bits, where the amount of
bits varies depending on the variant. The node header, stem:[h], is formally
defined as:

[stem]
++++
h = v_(N,1)^(0..x)||l_(N,1)^(x+1..7)||l_(N,2)^(0..7)||l_(N,3)^(0..7)||...
++++
...
h = {
    ...
    (0000 0000,"Empty"),
    (0001 0000,"Reserver for compact encoding")
    :}
++++
