...
\todo{TODO: this section is still heavily under construction. key questions about
...
\begin{enumerate}
    ...
    \SubItem{All pruned claims should have
    ...
    \SubItem{Assign claims in equal balance across all cores if rebalancing, and
...
\begin{itemize}
    ...
    queue.
    \SubItem{Fails if any parathread claim on the same parathread is
    currently indexed.}
    \SubItem{Fails if the queue length is >=
    ...
    \SubItem{\verb|next_core| is then
    updated by adding 1 and taking it modulo \verb|config.parathread_cores|.}
    \SubItem{The claim is then added to the claim index.}
    ...
    returned.
    \SubItem{All freed parachain cores should be assigned to their
    respective parachain}
    \SubItem{All freed parathread cores should have the
    claim removed from the claim index.}
    \SubItem{All freed parathread cores
    ...
    \SubItem{Fails if any given cores were not scheduled.} 
    \SubItem{Fails if the
    ...
    \SubItem{Since both the availability cores
