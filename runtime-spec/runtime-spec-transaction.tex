...
\begin{definition}
    ...
    is only valid within certain periods of the ongoing Polkadot lifetime. The
    extrinsic is not constrained to a specific block number, but is also valid
    for many different blocks where conditions apply.
    \newline
    ...
    The extrinsic author uses the number of the most recent block, $H_i(B)$,
    and specifies the desired period in order the calculate the phase:
    ...
    \ref{sect-mortality-encoding}). When a Polkadot validator tries to verify
    the extrinsic, it will check the mortality by validating whether the
    following condition applies:

    \[
        if\ H_i(B')\ mod\ M_{per} = M_{pha}
    \]

    where $H_i(B')$ is the number of the most recent block of the Polkadot
    validator. If the condition is true then the extrinsic is valid.
    ...
    \begin{itemize}
        \item 10'000, since $10'000\ mod\ 256 = 16$
        \item 10'256, since $10'256\ mod\ 256 = 16$
        \item 10'512, since $10'512\ mod\ 256 = 16$
        \item 10'768, since $10'768\ mod\ 256 = 16$
        \item ...
    \end{itemize}
