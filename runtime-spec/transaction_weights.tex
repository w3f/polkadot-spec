...
The Polkadot Runtime specifies a \verb|MaxRegistrars| constant, which will
prevent the list of registrars of reaching an undesired length. The benchmarking
implementation of the Polkadot Runtime function $request_judgement$ can be
defined as follows:

\begin{algorithm}[H]
  \caption{Run multiple benchmark iterations for $request_judgement$ Runtime function}
  \SetAlgoLined
...
\verb|MaxNominatorRewardedPerValidator|
HistoryDepth

\begin{algorithm}[H]
  \caption{Run multiple benchmark iterations for $payout_stakers$ Runtime function}
  \SetAlgoLined
  \KwResult{\textit{collection:} a collection of time measurements of all
  benchmark iterations}
  \BlankLine
  \Fn{\textsc{Main}}{
    \KwInit{collection = \{\};}\\
    \textsc{Pollute-Storage()};\\
    \For{$amount \leftarrow 1$ \KwTo $MaxNominators$ \KwBy $1$}{
