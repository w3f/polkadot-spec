...
by \syed{the}{a?} Polkadot public referendum, which can \syed{judge}{could you explain
  judge here more, is it just validating true false for all fields? some fields?} identities and therefore
incentivizes a reputation model. The judgement itself is done offchain. The
registrars \syed{rating}{what rating}, however, is saved onchain, directly in the corresponding
Identity Info. It's also note worthy that Identiy Info can contain additional
fields, set manually by the corresponding account holder. \syed{}{maybe an example of identity fields corresponding to an account clear things up}
\newline
...
\syed{}{I would put the signature/prototype of the function here so the reader knows which parameter you
send to the function. This applies to all examples please.}

If this function is called
by a \syed{previously judged user}{ by a specific registerar} it implies that the Identity Info should be
rejudged.
...
The Identity Info contains all fields that have a \syed{value set}{what is a value set}, in an ordered form.
It then proceeds to search all \syed{those entries}{which entries/clarify} for the specified registrar index. \syed{}{so the identity info is read from the state storage right? then maybe clarify that as well}
If the entry can be found, the corresponding value is \syed{updated}{ isn't there any kind of authentication of the registerar judgement?} to the value
...
maintaining sorted order. \syed{This results in memory reallocation}{ which results in considerable resource consumption?}.
...
\syed{In the end, the function ensures that the sender of the extrinsic has the
required balance in order to pay the fee, a balance that is adjustable and
determined by the Runtime itself}{what if an attacker submit lots of zero balanace account for judgement, while the transaction fails still they succeed in wasting the runtime time}. Then, an event is created to insert the Identity Info into storage. The creation of events is lightweight, but its execution is what will actually commit the state chgnanges.
...
\syed{}
     {
   Usage: The use of a plural noun after amount of (an amount of
   bananas; the amount of refugees) should be avoided: a quantity of
   bananas; the number of refugees
   http://thefreedictionary.com/amount
}
\begin{itemize}
  \item Varying \syed{amount}{ does this mean number/count? } of registrars.
  \item Varying \syed{amount}{ same as above }  of preexisting accounts in storage.
  \item \syed{The specified registrar}{why does it matter} is searched for in the Identity Info.
  ...
  can vary.}
  \item \syed{Varying sizes of Identity Info, including additional fields.}{ maybe we can be more accurate here }
  \item It is legitimate to introduce additional weights for changes the sender
  has influence over, such as the additional fields in the Identity Info.\syed{}{could you explain more, I don't get it why.}
\end{itemize}

\subsubsection{\syed{Preliminary Work}{I would call it Benchmarking Framework instead}}
...
\begin{algorithm}[H]
  ...
  \SetAlgoLined
  \KwResult{\syed{\textit{collection:} a collection of time measurements of all
  benchmark iterations}{the algorithm should return $\TWF$ be average or worst case of the collection to keep the document consistent}}
  \BlankLine
  \Fn{\textsc{Main}}{
    \KwInit{collection = \{\};}\\
    ...
    \Return{\syed{$collection$}{$\TWF$}}
  }
\end{algorithm}

\begin{itemize}
  \item \textsc{\syed{Pollute-Storage}{I would explain this process in the general section before going into example as it seems is just a condition for all test which touch storage} \SubItem{As clarified in Section
  ...
  \SubItem{Creates \syed{$amount$ of registrars}{no ``amount of'' before plural names} and inserts those records into storage.}
  ...
  \item \textsc{\syed{Random($num$)}{same here, I'd move the explanation of super general functions before the example section}}
...
not higher then the current era (as it is not in the future) and is within the allowed range 
also known as "history depth", as specified by the Runtime. After that, it fetches the era
payout from storage and additionally verifies whether the specified account is
indeed a validator and receives the corresponding \syed{"Ledger"}{perhaps don't drop jurgen without explanation, does this mean account?}.
...
\syed{The Ledger keeps information about the stash, such as actively bonded balance
and a list of tracked rewards. The function only retains the entries of the
history depth, and conducts a binary search for the specified era.}{could you decrypt this for people who are not familiar with Runtime jurgen}
...
As an optimizition, Runtime only fetches a list of the 64 highest staked nominators. Accordingly, any lower staked nominator gets no reward. 
...
Then, the nominators receive \syed{a}{is payout countable} payout. The functions loops over the nominator list, conducting an insertion into storage and a creation of a deposit event for
...
\end{itemize}
\syed{}{I'm surprised only 64 nominator per validotor get paid, what is the incentive of others
  to nominate then?}

\subsubsection{\syed{Preliminary Work}{better naming}}

\begin{definition}
  \label{defn-history-depth} {\b History Depth} indicated as {\textt MaxNominatorRewardedPerValidator} is a constant \syed{specified by the Polkadot Runtime}{so it is hard-coded or there is a algorithm to compute it/in that case I'd use calculated by?} which dictates the amount of Eras the reward system takes under consideration. \syed{}{I think consideration need to be defined}.
\end{definition}
...
\begin{itemize}
  \item \syed{\textsc{Pollute-Storage} \SubItem{As clarified in Section
  \ref{sec:blockchain-state}, the benchmarks should be run an a diverse range of
  storage states. From (mostly) empty databases to database with a (longer)
  history.}}{so this doesn't happens anymore better to remove it, they just count the number of read and write access and then they multiply it by a previously computed weight}
  ...
  \item \textsc{Timer($function$)} \syed{\SubItem{Measures the time from the start of
      the specified $function$ to its completion.}}{just refer to its original definition}
...
The $transfer$ function of the \syed{\textit{balances}}{texttt or textit let be consistent} module is designed to move the specified balance by the sender to the receiver.
