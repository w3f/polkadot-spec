...
The Polkadot Runtime specifies a \verb|MaxRegistrars| value, which will prevent the
list of registrars of reaching an undesired length.
\newline
...
\begin{algorithm}[H]
  ...
  \Fn{\textsc{Main}}{
    ...
    \For{$amount \leftarrow 1$ \KwTo $MaxRegistrars$ \KwBy $1$}{
      ...
      $caller \leftarrow$ \textsc{Create-Account("caller", $index$)};\\
      \textsc{Set-Balance($caller$, 100)};\\
      $time \leftarrow$ \textsc{Timer(Request-Judgement(Random($amount$), Random(100)))};\\
