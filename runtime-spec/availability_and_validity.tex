...
\begin{definition}
  \label{defn-para-proposal}
        {\b Parachain Block Proposal}, noted by $P^B_{rho}$is a candidate receipt for a parachain block $B$ for a parachain $\rho$ along with signatures for at least 2/3 of $mathcal{V}_\rho$  \syed{and no invalidity for it}{TBS how to deal with message of invalidity announcement}.
\end{definition}
...
For each parachain, /syed{we store}{what does it mean we store? this need to be clear. Is it a transaction \TBS?}:
...
\subsection{Processing on-chain avaialbility data}
This section explains how the availabalitiy attestations stored on the relay chain as described in Section \ref{se
...
\State We initialise a vector of counts with one entry for each parachain to zero. After executing the following algorithm we end up with a vector of counts  of the number of validators who think the latest candidates is available. 
   ...
   \item For each bit in $b_v$ and $bm_n$
        ...
        \end{itemize}
	\end{enumerate}	
\State For each count that is $>2/3$ of the number of validators, /syed{we set the candidates status to "available"}{what is the outside world implication of this setting, is there a message or just grandpa vote?}. Otherwise, if the candidate is at least $t$ blocks old, then we set its status to "unavailable".
...
/syed{}{So to be clear we are not annoucing unavailability we just keep it for grand pa vote}

\section{Publishing Attestations}
\label{sect-publishing-attestations}
\syed{}{this is out of place. We can mentioned that we have two type of (validity) attestetions in the intro but we just need to spec each attetstion in its relevant section (which we did with the canadidate receipt).
...
\syed{}{\todo{so assignees are not annoucing their assignment just the result of the approval check I assume}
