...
Collators produce a candidate by issuing a Collation as defined in Definition
\ref{defn-collation}. Validators verify the validity of the received Collation
as described in Algorithm \ref{algo-primary-validation}.
\todo{@fabio: confirm this} Validators back the validity respectively the
invalidity by extending the Collation into a Candidate Receipt as defined in
Definition \ref{defn-candidate-receipt} and communicate it by issuing a
Statement as defined in Definition \ref{defn-gossip-statement}.
\newline
...
\begin{definition}
  \label{defn-collation}
  A \textbf{Collation} contains the PoV block and enough data in order for any
  validator to verify its validity. A Collation is a tuple of the following
  format:
  \[
...
\begin{definition}
  ...
  Where $H_i(B)$ is the block number of parachain block $B$, $H_p(B)$ the 32-byte Blake2 hash of
  the parent block header of $B$ and $H_r(B)$ represents the root of the post-execution
  state.
...
\begin{definition}
  ...
  A \textbf{Candidate Receipt} is an extension of a Collation as defined in Definition \ref{defn-collation}
