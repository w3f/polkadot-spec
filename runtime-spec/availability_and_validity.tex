...
\todo{@fabio: delete this paragraph}
Primary validity checking refers to the process of parachain validators as
defined in Definition \ref{defn-parachain-validator} validating a parachain's
PoV block as explained in Algorithm \ref{algo-primary-validation}.
\newline
...
\subsection{Candidate Receipt}

Collators vouch for a candidate by issuing a Candidate Receipt, as defined in
Definition \todo{@fabio}. Validators verify the validity of the received Candidate Receipt
...
\begin{definition}
  A \textbf{Candidate Receipt} is a tuple of the following format:
  \[
  (p_{id}, h_{B_{relayparent}}, Head_p, C_{id}, Sig^{Collator}_{SR25519}, h_{PoV}, Commit)
  \]

  where each value represents:
  \begin{itemize}
    \item $p_{id}$: the Parachain this candidate is for.
    \item $h_{B_{relayparent}}$: the hash of the relay-chain block this candidate should be executed in the context of.
    \item $Head_p$: the head-data as defined in Definition \todo{@fabio}.
    \item $C_{id}$: the Collator relay-chain account ID as defined in Definition \todo{@fabio}.
    \item $Sig^{Collator}_{SR25519}$: the signature on the 256-bit Blake2 hash of the block data by the collator.
    \item $h_{PoV}$: the hash of the PoV block
    \item $Commit$: Commitments made as a result of validation, as described in \todo{@fabio}.
  \end{itemize}
...
\begin{definition}
  ...
  (h_b, h_bc, B)
  \]
  where $h_b$ is the block hash of the relay chain being referred to and $h_bc$ is the
  hash of some candidate localized to the same Relay chain block, whose PoV block is $h_bc$.
  $B$ is the PoV block itself.
  \todo{@fabio: adjust sentencing}
\end{definition}
...
\begin{definition}
  ...
  \]
  where $h_b$ is the block hash of the relay chain being referred to. $Stmt$ is a statement
  as defined in Definition X. \todo{@fabio: define Statement} $Sig_{SR25519}$ is the signature
  of the validator and $id_{\mathbb{V}}$ reffers to the index of validator according to the
  authority set. \todo{@fabio: define authority set}
  \newline

  $Stmt$ refers to a statement the validator wants to make about a certain candidate.
  $Stmt$ is a varying datatype as defined in X \todo{@fabio: define} can be one of the
  following values:

  \begin{equation}
    Stmt =
    \begin{cases}
      0, & \text{New candidate being seconded, followed by \textbf{Candidate Receipt}} \\
      1, & \text{Validity of a candidate, followed by 32-byte hash} \\
      2, & \text{Invalidity of a candidate, followed by 32-byte hash}
    \end{cases}
  \end{equation}
\end{definition}
...
second only one candidate; this places an upper bound on the total number of candidates whose validity
