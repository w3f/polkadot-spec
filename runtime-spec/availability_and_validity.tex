...
computation of $Er_B$, $v$ must construct the $\blobB$ Availability message
...
When validator $v$ receives its designated piece for \blobB it needs to
...
%$\blobB$ before checking.  Now $V$ knows which which nodes have their
%individual pieces, thanks to their availability announcements.  It thus follows
%from our 2/3rd honest assumption that $V$ could always reconstruct $\blobB$ by
...
%$\blobB$, as well as approval checkers who already approved $\blobB$.  So $V$
%could first contact some node that possesses all of $\blobB$, and only then
...
%reconstructing $\pieces_B$ from arbitrary pieces or from $\blobB$ itself.  It
%remains plausible $V$ avoids some networking overhead by asking for $\blobB$
%though.  We think a first implementation could reasonably target reconstructing
%$\pieces_B$ from arbitrary pieces, while leaving requests for the full $\blobB$
...
the subsequently recover the original $\blobB$.
