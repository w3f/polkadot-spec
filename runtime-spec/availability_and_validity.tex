...
\begin{definition}
  ...
  \[
  (p_{id}, h_b({B_{^{relay}_{parent}}}), Head_p, C_{id}, Sig^{Collator}_{SR25519}, h_b({PoV_B}), \mathbb{C})
  \]

  where each value represents:
  \begin{itemize}
    \item $p_{id}$: the Parachain this candidate is for.
    \item $h_{B_{^{relay}_{parent}}}$: the hash of the relay-chain block this
    candidate should be executed in the context of.
    \item $Head_p$: the head-data as defined in Definition \todo{@fabio}.
    \item $C_{id}$: the Collator relay-chain account ID as defined in Definition
    \todo{@fabio}.
