...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    ...
    \State \textbf{Init} $Shards = $ \textsc{Make-Shards}($\paraValidSet, v_B$)
    \Statex
    \Statex // Results in an array of tuples
    \State \textbf{Init} $Pieces$
    \State \textbf{Init} $counter = 0$
    \For{$shard \in Shards$}
      \State \textsc{Add}($Pieces, (shard, counter,$ \textsc{Blake2}($shard$)))
      \State $counter \leftarrow counter + 1$
    \EndFor
    \Statex
    \Statex \todo{@fabio: SCALE encode?}
    \State \Return $Pieces$
  \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    ...
    \Statex
    \Statex // Results is an array of arrays,
    ...
    \For{$(chunk, blank) \in$ (\textsc{Take}$(Enc_{SC}(v_B), Shard_{len}), Shards)$}
      \State \textbf{Init} $len =$ \textsc{Min}($Shard_{len}, |chunk|$)
