...
<\body>
  ...
  The Changes Trie as described in section <reference|defn-changes-trie>
  requires a mechanism to track changes that are made per block, therefore
  every call to the Host API that inserts and manipulates data in the state
  storage is first accumulated in an isolated environment and only committed
  after the Runtime calls <verbatim|ext_storage_changes_root> as described in
  section <reference|sect-ext-storage-changes-root>.
  <verbatim|ext_storage_changes_root> uses the entries in that isolated
  environment to generate the Changes Trie, then will be wiped clean and all
  the changes will be committed to the primary state storage.
...
<\references>
  <\collection>
    ...
    <associate|auto-335|<tuple|Tec19|?>>
    <associate|auto-336|<tuple|Tec19|?>>
