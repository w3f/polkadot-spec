...
where stem:[M_c] is the collator message (<<net-msg-collator-protocol>>).
****
...
* stem:[H] is the hash of the parachain block (<<defn-parablock>>).
* stem:[S] is a full statement (<<net-msg-full-statement>>).
****
...
<<sect-candidate-backing-statements>>. The statement distribution message,
...
<<sect-availability-assingment-criteria>>. The approval distribution message,
stem:[M], is a varying datatype of the following format:
...
(<<defn-pov-block>>) and stem:[D_{pv}] is the persisted validation data
...
(<<defn-pov-block>>), stem:[B]. This type does not notify the client about a
