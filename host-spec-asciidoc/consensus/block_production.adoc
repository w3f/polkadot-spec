...
lottery secret key and latexmath:[$n$] is the index of the epoch for whose slots
...
The *Winning threshold* denoted by stem:[T_(E_n)] is the threshold that is used
alongside the result of Algorithm
link:#algo-block-production-lottery[[algo-block-production-lottery]] to decide
if a block producer is the winner of a specific slot. stem:[T_(E_n)] is
