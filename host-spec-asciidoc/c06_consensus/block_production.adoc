...
the Runtime entry `BabeApi_configuration` (TODO <<sect-rte-babeapi-epoch>>) at
...
submit those to the Runtime as described in TODO
...
value returned by calling `BabeApi_configuration` (TODO
<<sect-rte-babeapi-epoch>>). For efficiency reasons, it is generally updated by
the Runtime through the _next config data_ consensus message
(<<defn-consensus-message-digest>>) in the digest of the first block of an epoch
for the next epoch.
