...
Section link_sect-networking-external-docs[4.2].
...
over _mplex_. Section link_sect-protocols-substreams[4.7] describes the
...
link_sect-network-messages[4.8].
...
the node’s _PeerId_ as defined in Definition link_defn-peer-id++defn-peer-id]]
is derived from its public key. _PeerId_ (link_defn-peer-id++defn-peer-id]]) is
used to identify each node when they are discovered in the course of the
discovery mechanism described in Section link_sect-discovery-mechanism[4.4].
...
as described in Section link_sect-protocols-substreams[4.7].
...
link_sect-protocols-substreams[4.7] describes the usage of each type in
more detail:
...
See Section link_sect-protocols-substreams[4.7] for more information.
...
link_sect-encryption-layer[4.6].
...
link_sect-connection-establishment[4.5].
...
+
The messages are specified in Section link_sect-msg-block-announce[4.8.1].
...
+
The messages are specified in Section link_sect-msg-block-request[4.8.2].
...
+
The messages are specified in Section link_sect-msg-transactions[4.8.3].
...
+
The messages are specified in Section link_sect-msg-grandpa[4.8.4].
+
...
BEEFY statements, as described in Section link_sect-grandpa-beefy[6.5], to
connected peers. This is a _Notification_ substream.
+
The messages are specified in Section link_sect-msg-grandpa-beefy[4.8.4.5].
