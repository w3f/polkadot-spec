...
described in Section link:#sect-grandpa-beefy[6.5]. Those messages are sent over
...
link:#defn-authority-set-id[[defn-authority-set-id]]) in stem:[C]. Individual
items are of the type as defined in Definition
link:#defn-option-type[[defn-option-type]] which can contain a signature of a
