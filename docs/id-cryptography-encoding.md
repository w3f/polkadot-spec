...
In Polkadot, VRFs are used for the BABE block production lottery by [Block-Production-Lottery](sect-block-production#algo-block-production-lottery) and the parachain approval voting mechanism ([Section 8.5](chapter-anv#sect-approval-voting)). The VRF uses mechanism similar to algorithms introduced in the following papers:
...
###### Table 2. List of the public key scheme which can be used for an account key {#tabl-account-key-scheme}
