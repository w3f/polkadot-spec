...
fn get_wasm_blob() -> Vec<u8> {
    ...
    File::open("build/test/testers/rust-tester-legacy/x86_64-unknown-linux-gnu/debug/wbuild/wasm-blob/wasm_blob.compact.wasm")
    // for `cargo` inside rust-tester directory
    //File::open("test/testers/rust-tester/target/debug/wbuild/wasm-blob-legacy/wasm_blob_legacy.compact.wasm")
