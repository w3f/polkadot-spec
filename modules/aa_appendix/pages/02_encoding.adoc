...
where stem:["Enc"_("SC")^("Len")] is defined in xref:02_encoding.adoc#defn-sc-len-encoding[Def.].
====
...
xref:02_encoding.adoc#defn-varrying-data-type[Def.], we have:
...
defined in xref:02_encoding.adoc#defn-scale-variable-type[Def.] and the data type it represents; the
...
values defined in xref:01_background/definitions.adoc#defn-little-endian[Def.].
====
