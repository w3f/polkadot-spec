...
* `data`: a pointer-size (xref:intro.adoc#defn-runtime-pointer[Def.]) indicating the
...
* `data`: a pointer-size (xref:intro.adoc#defn-runtime-pointer[Def.]) indicating the enumerated
...
integers (xref:02_encoding.adoc#defn-sc-len-encoding[Def.]).
* `result`: a 32-bit pointer to the buffer containing the 256-bit trie root
result.
