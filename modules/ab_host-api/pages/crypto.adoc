...
==== ECDSA Verify Error
****
*EcdsaVerifyError* is a varying data type (xref:02_encoding.adoc#defn-varrying-data-type[Def.]) that
...
(xref:crypto.adoc#defn-key-type-id[Def.]).
* `return`: a pointer-size (xref:intro.adoc#defn-runtime-pointer[Def.]) to an SCALE encoded
256-bit public keys.
...
* `key_type_id`: a 32-bit pointer to the key type identifier
(xref:crypto.adoc#defn-key-type-id[Def.]).
* `seed`: a pointer-size (xref:intro.adoc#defn-runtime-pointer[Def.]) indicating the SCALE encoded
_Option_ value (xref:02_encoding.adoc#defn-option-type[Def.]) containing the BIP-39 seed which must be
valid UTF8.
...
* `msg`: a pointer-size (xref:intro.adoc#defn-runtime-pointer[Def.]) indicating the
message that is to be signed.
* `return`: a pointer-size (xref:intro.adoc#defn-runtime-pointer[Def.]) indicating the SCALE
encoded _Option_ value (xref:02_encoding.adoc#defn-option-type[Def.]) containing the 64-byte signature.
...
(xref:crypto.adoc#sect-ext-crypto-finish-batch-verify[Sect.]).

The verification extension is explained more in detail in
(xref:crypto.adoc#sect-ext-crypto-start-batch-verify[Sect.]).
...
* `key_type_id`: a 32-bit pointer to the key identifier (xref:crypto.adoc#defn-key-type-id[Def.]).
* `seed`: a pointer-size (xref:intro.adoc#defn-runtime-pointer[Def.]) indicating the SCALE encoded
_Option_ value (xref:02_encoding.adoc#defn-option-type[Def.]) containing the BIP-39 seed which must be
valid UTF8.
* `return`: a 32-bit pointer to the buffer containing the 256-bit public key.
...
* `key_type_id`: a 32-bit pointer to the key type identifier (xref:crypto.adoc#defn-key-type-id[Def.]).
* `return`: a pointer-size (xref:intro.adoc#defn-runtime-pointer[Def.]) indicating the
...
message that is to be signed.
* `return`: a pointer-size (xref:intro.adoc#defn-runtime-pointer[Def.]) indicating the SCALE
encoded _Option_ value (xref:02_encoding.adoc#defn-option-type[Def.]) containing the signature. The
...
verification is registered, this function will push the data to the batch and
return immediately. The caller can then get the result by calling
`ext_crypto_finish_batch_verify` (xref:crypto.adoc#sect-ext-crypto-finish-batch-verify[Sect.]).
...
the message.
* `return`: a pointer-size (xref:intro.adoc#defn-runtime-pointer[Def.]) indicating the SCALE
encoded _Result_ (xref:02_encoding.adoc#defn-result-type[Def.]). On success it contains the 64-byte
recovered public key or an error type (xref:crypto.adoc#defn-ecdsa-verify-error[Def.]) on failure.
...
(xref:crypto.adoc#defn-ecdsa-verify-error[Def.]) on failure.
...
with `ext_crypto_ed25519_verify` (xref:crypto.adoc#sect-ext-crypto-ed25519-verify[Sect.]),
`ext_crypto_sr25519_verify` (xref:crypto.adoc#sect-ext-crypto-sr25519-verify[Sect.]) or
`ext_crypto_ecdsa_verify` (xref:crypto.adoc#sect-ext-crypto-ecdsa-verify[Sect.]). Verification will
...
(xref:crypto.adoc#sect-ext-crypto-start-batch-verify[Sect.]) was not called.
