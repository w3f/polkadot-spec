...
described in xref:modules/grandpa.adoc#sect-rte-grandpa-auth[Sect.]. We refer to stem:[bbb V_B] as
...
(xref:05_consensus/common.adoc#defn-consensus-message-digest[Def.]). Starting with the value of zero at genesis,
...
* stem:["id"_(bbb V)]: is the authority set ID (xref:05_consensus/finality.adoc#defn-authority-set-id[Def.]).
...
(xref:03_transition/state_replication.adoc#defn-block-header-hash[Def.]) and the block number (xref:03_transition/state_replication.adoc#defn-block-header[Def.]).
****
...
Voting is done by means of broadcasting voting messages (xref:04_networking/messages.adoc#sect-msg-grandpa[Sect.])
to the network. Validators inform their peers about the block finalized in round
stem:[r] by broadcasting a commit message (xref:05_consensus/finality.adoc#algo-grandpa-round[Algo.]).
****
...
(xref:05_consensus/finality.adoc#defn-vote[Def.]).
...
set Id (xref:05_consensus/common.adoc#defn-authority-list[Def.]).
****
...
(xref:05_consensus/finality.adoc#defn-sign-round-vote[Def.]) of voter stem:[v_i in bbb V_B] broadcasted during
...
(xref:05_consensus/block_finalization.adoc#defn-finalized-block[Def.]) of block stem:[B] by actively participating in the
voting process. That is by invoking Algorithm as defined in
xref:05_consensus/finality.adoc#algo-grandpa-round[Algo.].

The GRANDPA protocol dictates how an honest voter should vote in each sub-round,
which is described by Algorithm as defined in xref:05_consensus/finality.adoc#algo-grandpa-round[Algo.]. After
...
TODO xref:modules/grandpa.adoc#sect-grandpaapi_submit_report_equivocation_unsigned_extrinsic[Sect.].
...
the block chosen by Algorithm as described in xref:05_consensus/finality.adoc#algo-grandpa-ghost[Algo.]:
...
gossiped messages including those mentioned in xref:05_consensus/block_finalization.adoc#defn-finalized-block[Def.].
...
(xref:05_consensus/common.adoc#sect-consensus-message-digest[Sect.]). When Authorities process such messages they
...
* stem:["Derive-Primary"] is described in xref:05_consensus/finality.adoc#algo-derive-primary[Algo.].
* The condition of _completablitiy_ is defined in xref:05_consensus/finality.adoc#defn-grandpa-completable[Def.].
* stem:["Best-Final-Candidate"] function is explained in xref:05_consensus/finality.adoc#algo-grandpa-best-candidate[Algo.].
* stem:["Attempt-To-Finalize-Round"(r)] is described in <<algo-attempt-to–finalize>>.
* stem:["Finalizable"] is defined in xref:05_consensus/finality.adoc#algo-finalizable[Algo.].
****
...
(xref:05_consensus/block_finalization.adoc#defn-finalized-block[Def.]).
* stem:[#V_("obs"(v))^(r,pv)(B)] is defined in xref:05_consensus/finality.adoc#defn-observed-votes[Def.].
****
...
xref:05_consensus/finality.adoc#defn-grandpa-completable[Def.].
****

Note that we might not always succeed in finalizing our best final candidate due
to the possibility of equivocation. The example in xref:05_consensus/finality.adoc#exmp-candid-unfinalized[Decl.]
...
condition of Algorithm as described in xref:05_consensus/finality.adoc#algo-grandpa-round[Algo.] is
...
Both scenarios unblock the Algorithm as defined in xref:05_consensus/finality.adoc#algo-grandpa-round[Algo.],
