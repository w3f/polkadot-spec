...
is encapsulated in a Runtime (xref:01_background/definitions.adoc#defn-state-machine[Def.]). For easy upgradability
this Runtime is presented as a Wasm blob. Nonetheless, the Polkadot Host needs
to be in constant interaction with the Runtime (xref:03_transition/runtime_interaction.adoc#sect-entries-into-runtime[Sect.]).

In xref:03_transition/extrinsics.adoc#sect-extrinsics[Sect.], we specify the procedure of the process where the
...
validity are both explained in xref:03_transition/state_replication.adoc#sect-state-replication[Sect.].

[#sect-entries-into-runtime]
=== Interacting with the Runtime

The Runtime (xref:01_background/definitions.adoc#defn-state-machine[Def.]) is the code implementing the logic of the chain.
...
defined in xref:03_transition/runtime_interaction.adoc#algo-runtime-interaction[Algo.].
...
xref:03_transition/runtime_interaction.adoc#algo-runtime-interaction[Algo.] are explained in xref:03_transition/runtime_interaction.adoc#notat-call-into-runtime[Decl.] and
xref:03_transition/state_replication.adoc#defn-set-state-at[Def.] respectively. stem:[R_B] is the Runtime code loaded from
stem:[S_B], as described in xref:03_transition/runtime_interaction.adoc#notat-runtime-code-at-state[Decl.], and stem:[RE_B] is
the Polkadot Host API, as described in xref:intro.adoc#nota-host-api-at-state[Decl.].
...
called. Accordingly, we define stem:[R_B] (xref:03_transition/runtime_interaction.adoc#notat-runtime-code-at-state[Decl.]).
...
storage API (xref:storage.adoc#sect-storage-api[Sect.]).
...
xref:03_transition/runtime_interaction.adoc#notat-call-into-runtime[Decl.] introduces the notation for calling the runtime entry
...
xref:03_transition/runtime_interaction.adoc#sect-runtime-send-args-to-runtime-enteries[Sect.].
****
...
(xref:allocator.adoc#sect-allocator-api[Sect.]) and the same allocator should be used for any other
...
encoded using SCALE codec described in xref:02_encoding.adoc#sect-scale-codec[Sect.]. Therefore all
...
(xref:02_encoding.adoc#sect-scale-codec[Sect.]) and copied into a section of Wasm shared memory managed
by the shared allocator described in xref:03_transition/runtime_interaction.adoc#sect-memory-management[Sect.].
...
validate a transaction using entry (xref:modules/txqueue.adoc#sect-rte-validate-transaction[Sect.]), it needs
...
`Core_execute_block` (xref:modules/core.adoc#sect-rte-core-execute-block[Sect.]) while Polkadot Host is
importing a block (xref:03_transition/state_replication.adoc#sect-block-validation[Sect.]).

For more information on managing multiple variant of state see
xref:03_transition/state_replication.adoc#sect-managing-multiple-states[Sect.].
