...
.<<defn-vrf-transcript, VRF Transcript>>
====
A *transcript*, or VRF transcript, is a STROBE object as defined in the STROBE
documentation, respectively section
https://strobe.sourceforge.io/specs/#object["5. State of a STROBE object"].

[stem]
++++
T = ("st","pos","pos"_("begin"),I_0)
++++
...
the initial STROBE state as described in the STROBE documentation.
Specifically, the `R` value is of value `166` and `X.Y.Z` is `1.0.2`
