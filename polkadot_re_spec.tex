...
\tmfloat{h}{small}{figure}{\begin{tmcode}[cpp]
  ...
  (export "ParachainHost_active_parachains" (func $ParachainHost_active_parachains))
  ...
  (export "GrandpaApi_grandpa_pending_change" (func $GrandpaApi_grandpa_pending_change))
  (export "GrandpaApi_grandpa_forced_change" (func $GrandpaApi_grandpa_forced_change))
...
storage. When traversing the Trie to a specific node, its key can be
reconstructed by concatenating the subsequences of the key which are stored
either explicitly in the nodes on the path or implicitly in their position as
a child of their parent.
...
\begin{definition}
  \label{defn-node-key}Each Node in the Trie is identified with a unique key
  $k_N$ such that $k_N$ is the shared prefix of the key of all the descendents
  of $N$ in the Trie. $k_N$ is divided into an {\tmstrong{aggregated prefix
  key}}, {\tmstrong{$\tmop{pk}_N^{\tmop{Agr}}$}}, aggregated by Algorithm
...
\begin{definition}
  \label{defn-nodetype}The State Trie is a radix-16 tree. As such, for any $N
  \in \mathcal{N}$ with key $k_N$, at least one of the following statements
  holds:
  \begin{itemizeminus}
  ...
  \end{itemizeminus}
  Conversly, if $(k, V)$ is an entry in the State Trie then there is a node $N
  \in \mathcal{N}$ such that $k_N$=k.
...
\begin{definition}
  ...
  \ref{defn-node-key}, \ref{defn-he} and \ref{defn-node-subvalue},
  respectively.
\end{definition}
...
{\state{$l \leftarrow l - 254$}}

{\state{$i \leftarrow i + 1 \END$}}

{\state{$\tmop{Head}_N, i \leftarrow l$}}}}
...
\begin{definition}
  ...
  \begin{itemize}
    \[ \begin{array}{l}
         \tmop{sv}_N \assign\\
         \left\{ \begin{array}{cc}
           \tmop{Enc}_{\tmop{SC}} (v) & \text{N is a leaf node}\\
           \nobracket \tmop{Enc}_{\tmop{LE}} (\tmop{ChildrenBitmap} (N)) \|H
           (N_{C_1}) \ldots \nobracket H (N_{C_n}) | | \tmop{Enc}_{\tmop{SC}}
           (v) & \text{N is a branch node}
         \end{array} \right.
       \end{array} \]
...
Where
\begin{itemize}
  \item $\tmop{Enc}_{\tmop{SC}}$ defined in Definition \ref{def-scale-codec}.
...
\begin{definition}
  {\tmstrong{Block signing key pair $(\tmop{sk}^s_j, \tmop{pk}^s_j)$}} is an
  ED25519 key pair which the block producer $\mathcal{P}_j$ signs by their
  account key (see Definition \ref{defn-account-key}) and is used to sign the
  produced block. Similarly {\tmstrong{block lottery key pair}}, noted by
  {\tmstrong{$(\tmop{sk}_j^v, \tmop{pk}^s_j)$}}, is a ED25519 key pair which
  is used by $\mathcal{P}_j$ to compute its lottery values. 
\end{definition}
...
\begin{definition}
  ...
  {\tmstrong{slot}}s sequentially indexed in each epoch. Each slot is awarded
...
$\tmop{sk}^v$ is the block producer lottery secret key and $n$ is the epoch
for whose slots the block producer is running the lottery.

\custombinding{4}{\noindent}\begin{tabular}{l}
  \hline
  \tmtextbf{Algorithm  4. }
  \label{algo-block-production}{\tmname{Block-production-lottery}}$(\tmop{sk}^v,
  n)$\\
  \hline
\end{tabular}
...
{\state{$(d, \pi) \leftarrow${\tmem{{\tmname{VRF}}}}($i, \tmop{sk}^v$)}}

{\state{$A [i] \leftarrow (i, d, \pi)${\END}}}
...
{\tmem{{\tmname{VRF}}}} functions, see Sections \ref{sect-randomness} and
\ref{sect-vrf} respectively.
...
\custombinding{8}{\noindent}\begin{tabular}{l}
  \hline
  \tmtextbf{Algorithm  8. } {\tmname{Attempt-To-Finalize-Round}}($r$)\\
  \hline
\end{tabular}
...
\begin{definition}
  \label{defn-he}Suppose that $\tmop{PK} = (k_1, \ldots, k_n)$ is a sequence
  of nibbles, then
  ...
  \begin{tabular}{l}
    ...
    $\left\{ \begin{array}{lll}
      ...
      \tmop{PK} = (k_1, \ldots, k_n) & \mapsto & \left\{ \begin{array}{l}
        \begin{array}{ll}
          (k_1 + 16 k_2, \ldots, k_{2 i - 1} + 16 k_{2 i}) & n = 2 i\\
          (k_1, k_2 + 16 k_3, \ldots, k_{2 i - 1} + 16 k_{2 i}, k_{2 i + 1}) &
          n = 2 i + 1
        \end{array}
      \end{array} \right.
    \end{array} \right.$
...
\begin{itemize}
  ...
  \item \tmverbatim{result}: \ an integer value equal to 0 indicating the
