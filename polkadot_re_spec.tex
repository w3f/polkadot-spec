...
\usepackage{amsmath,amssymb,calc,ifthen,alltt,capt-of}
...
(B)$)}}}{{\algorithmic{1\quad{\tmname{Verify-Block-Justification}}$(B,
\tmop{Just} (B))$
...
($\mathcal{V}$)}{{\algorithmic{1\quad$r \leftarrow 0$

2\quad$\mathcal{V}_{\tmop{id}} \leftarrow \tmop{ReadState} (' \tmop{AUTHORITY}
\_ \tmop{SET} \_ \tmop{KEY}')$

3\quad$\mathcal{V}_{\tmop{id}} \leftarrow \mathcal{V}_{\tmop{id}} + 1$
...
{\algorithm{\label{alg-grandpa-round}{\tmname{Play-Grandpa-round}}$(r)$}{{\algorithmic{1\quad$t_{r,
v} \leftarrow$Time
...
5\qquad{\tmname{Broadcast(}}\left.$M_{v_{\nosymbol}}^{r - 1,
\tmop{fin}}$()\right)

6\quad{\tmstrong{else}}

9\quad{\tmname{\quad Receive-Messages}}({\tmstrong{until}} Time $\geqslant
t_{r_, v} + 2 \times T$ {\tmstrong{or}}

{\hspace{3em}} {\tmname{Completable}}($r$))

10\qquad$L \leftarrow${\tmname{Received-as-Final()}} {\tmstrong{or}}
{\tmname{Best-Final-Candidate}}(
...
16\quad{\tmname{Receive-Messages}}({\tmstrong{until}} $B^{r,
\tmop{pv}_{\nosymbol}}_v \geqslant L$ {\tmstrong{and}} (

{\hspace{3em}}Time $\geqslant t_{r_, v} + 4 \times T${\tmstrong{ or
}}{\tmname{Completable}}($r$)))

12\quad{\tmname{Broadcast($M_v^{r, \tmop{pc}}$(Best-Final-Candidate($r$))}}

13\quad{\tmname{Play-Grandpa-round}}($r + 1$)}}}}

{\algorithm{\label{alg-grandpa-best-candidate}{\tmname{Best-Final-Candidate}}($r$)}{{\algorithmic{1\quad$\mathcal{C}_{\nosymbol}
...
5\qquad{\tmstrong{return }}$E \in \mathcal{C}: H_n (E) = \max \{H_n (B') : B'
\in \mathcal{C}\}$}}}}

{\algorithm{\label{alg-completable-round}{\tmname{Completable}}($r$)}{{\algorithmic{1\quad$E
\leftarrow${\tmname{Best-Final-Candidate}}($r$)
...
6\qquad{\tmstrong{return}} {\tmname{False}}}}}}

{\algorithm{{\tmname{FinalizeRound}}($r$)}{{\algorithmic{1\quad$L
\leftarrow${\tmname{Last-Finalized-Block}}

2\quad$E \leftarrow${\tmname{Best-Final-Candidate}}($r$)

2\quad{\tmstrong{if}} $E \geqslant L$ {\tmstrong{and}} $V^{r - 1,
\tmop{pc}}_{\tmop{obs} (v)}^{\nosymbol}_{\nosymbol} (E) > 2 / 3 |\mathcal{V}|$

3\qquad{\tmname{Last-Finalized-Block}}$\leftarrow B^{r, \tmop{pc}}$

4\qquad{\tmstrong{if}} $M_v^{r, \tmop{Fin}} (E)
\nin${\tmname{Received-Messages}}:

5{\hspace{3em}}{\tmname{Broadcast}}($M_v^{r, \tmop{Fin}} (E)$)}}}}
