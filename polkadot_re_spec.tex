...
Definition \ref{defn-chain-subchain} gives the means to high light various
branchs of the block tree.
...
\begin{notation}
  ...
  $\tmop{sc}_n$. $\tmop{sc}_n$ is set to ??? at the genesis. For a given block
  $B$, we use the notation {\tmstrong{$s_B$}} to refer to the slot during
  which $B$ has been produced. Conversely, for slot $s$, $\mathcal{B}_s$ is
  the set of Blocks generated at slot $s$.
\end{notation}
...
\begin{definition}
  ...
  winner of a specific slot. $\tau$ is initially set to ???.
\end{definition}
...
{\state{{\tmname{Deepest-Leaf($\tmop{BT}$)}}}}

{\state{{\FORIN{$B_i$}{{\tmname{SubChain}}({\tmname{}} $- \tmop{SlTl}$,
{\tmname{Last-Finalized-Slot}})}}}}
...
\custombinding{11}{\noindent}\begin{tabular}{l}
  \hline
  \tmtextbf{Algorithm  11. } {\tmname{Attempt-To-Finalize-Round}}($r$)\\
  \hline
\end{tabular}
...
Computes the {\tmem{xxHash64}} algorithm (see
{\cite{collet_extremely_nodate}}) twice initiated with seeds 0 and 1 and
applied on a given byte array and outputs the concatenated result.
...
\begin{thebibliography}{DGKR18}
  ...
  \bibitem[Col]{collet_extremely_nodate}Yann Collet. {\newblock}Extremely fast
