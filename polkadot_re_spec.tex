...
\custombinding{1}{\noindent}\begin{tabular}{l}
  \hline
  \tmtextbf{Algorithm  1. } {\tmname{Import-and-Validate-Block($B, \tmop{Just}
  (B)$)}}\\
  \hline
\end{tabular}
...
re-arranged and hashed in a {\tmem{modified Merkle Patricia Tree}}, which
hereafter we refer to as the {\tmem{{\tmstrong{Trie}},}} in order to compute
the hash of the whole state storage consistently and efficiently at any given
time.
...
Each key value identifies a unique node in the tree. However, a node in a tree
might or might not be associated with a key in the storage. Similar to a radix
trie, when traversing the Trie, subsequences of the key, are stored in the
...
encode $k$ in a way consistent with the Trie structure. Because each node in
...
\begin{itemize}
  \item {\tmstrong{Leaf:}} It is a childless node that stores the remaining
  subsequence of $\tmop{KeyEncode} (k)$ not accounted for in the path to the
  node, as well as the value associated with that key.
  
  \item {\tmstrong{Extension:}} It stores the subsequence of KeyEncode($k$)
  of a size larger than 1 (i.e. 4 bits), which is at least shared with two or
  more keys in the Trie. It always has one child. It does not store a value.
  
  \item {\tmstrong{Branch:}} It has up to 16 children. It stores no key. It
  accounts for one nibble of the key corresponding to the path passing through
  each of its children. It optionally stores a value of the key corresponding
  to the path traversed to the node.
\end{itemize}
Accordingly, we define:

\begin{definition}
  \label{defn-nodetype}We define {\tmstrong{$\tmop{NodeType}$}} function to be
  \[ \begin{array}{lllll}
       \tmop{NodeType} & : & \mathcal{N} & \rightarrow & \{ \tmop{Leaf},
       \tmop{Extension}, \tmop{Branch} \}\\
       &  & N & \mapsto & \tmop{The} \tmop{type} \tmop{of} \tmop{node} N
     \end{array} \]
\end{definition}

\begin{definition}
  The {\tmstrong{partial key}} of node N of length $j \leqslant 380$ to be
  \[ \tmop{pk}_N \assign (k_{\tmop{enc}_i}, \ldots, k_{\tmop{enc}_{i + j}})
  \]
  a subsequence of a key $k$
  \[ \tmop{KeyEncode} (k) = (k_{\tmop{enc}_1}, \ldots, k_{\tmop{enc}_{i - 1}},
     k_{\tmop{enc}_i}, \ldots, k_{\tmop{enc}_{2 n}}) \]
  corresponding to a key-value paired $(k, v)$ in the State storage such that
  $\tmop{pk}_N$ is stored in $N$.
\end{definition}

Consequently, for an extension node $j \geqslant 2$ and for a branch node $j =
0$.
...
\begin{definition}
  ...
  \[ \tmop{NodePrefix} (N) \assign \left\{ \begin{array}{lll}
       ...
       128 &  & N \text{ is an extension node}\\
...
\begin{definition}
  ...
  \begin{itemize}
    ...
    \[ \tmop{Enc}_{\tmop{Node}} (N) \assign \tmop{Enc}_{\tmop{len}} (N) ||
       \tmop{Enc}_{\tmop{HE}} (\tmop{pk}_N) || \tmop{Enc}_{\tmop{SC}} (v) \]
    \item N is an extension node:
    \[ \tmop{Enc}_{\tmop{Node}} (N) : = \tmop{Enc}_{\tmop{len}} (\nobracket N)
       \| \tmop{Enc}_{\tmop{HE}} (\tmop{pk}_N) | | \tmop{Enc}_{\tmop{SC}} (H
       (C)) \]
    \item N is a branch node:
    \[ \begin{array}{l}
         \text{Enc\tmrsub{Node}(N)} \assign\\
         \nobracket \tmop{NodePrefix} (N) || \tmop{Enc}_{\tmop{LE}}
         (\tmop{ChildrenBitmap} (N)) \| \tmop{Enc}_{\tmop{SC}} (v) || \\
         \tmop{Enc}_{\tmop{SC}} (H (N_{C_1})) \ldots \tmop{Enc}_{\tmop{SC}} (H
         (N_{C_n}))
       \end{array} \]
  \end{itemize}
\end{definition}
...
\begin{itemize}
  \item $\tmop{Enc}_{\tmop{len}}, \tmop{Enc}_{\tmop{HE}}$ and
  $\tmop{Enc}_{\tmop{SC}}$ are encdoings defined in Section
  \ref{sect-encoding}.
  
  \item $\tmop{Enc}_{\tmop{LE}}$ is the little-endian encoding defined in
  Definition \ref{defn-little-endian}.
  
  \item $C$ is the unique child of the extension node $N$.
  
  \item $\tmop{ChildrenBitmap} (N) \assign (b_{15} b_{14} \ldots, b_1, b_0)_2$
  where bit $b_i = 1$ if $N$ has a child with partial key $i$.
  
  \item $N_{C_1} \ldots N_{C_n}$ with $n \leqslant 16$ are the children nodes
  of the branch node $N$.
\end{itemize}
\begin{definition}
...
\custombinding{4}{\noindent}\begin{tabular}{l}
  \hline
  \tmtextbf{Algorithm  4. }
  \label{alg-grandpa-best-candidate}{\tmname{Best-Final-Candidate}}($r$)\\
  \hline
\end{tabular}
...
\custombinding{5}{\noindent}\begin{tabular}{l}
  \hline
  \tmtextbf{Algorithm  5. } {\tmname{Attempt-To-Finalize-Round}}($r$)\\
  \hline
\end{tabular}
...
\begin{definition}
  \label{def-key-len-enc}Let $N$ be a leaf or an extension node in the storage
  state trie with Partial Key $\tmop{PK}_N$. We define the {\tmstrong{Partial
  key length encoding}} function, formally referred to as
  $\tmop{Enc}_{\tmop{len}} (N),$ as follows:
  \[ \begin{array}{ll}
       \tmop{Enc}_{\tmop{len}} (N) & \assign\\
       \tmop{NodePrefix} (N) & +\\
       \left\{ \begin{array}{lcl}
         (\| \tmop{PK}_N \|) &  & \| \tmop{PK}_N \|< \tmop{BigKeySize}
         (\tmop{NodeType} (N))\\
         (127) || (\| \tmop{PK}_N \|- \tmop{BigKeySize} (\tmop{NodeType} (N)))
         &  & \| \tmop{PK}_N \| \geqslant \tmop{BigKeySize} (\tmop{NodeType}
         (N))
       \end{array} \right. & 
     \end{array} \]
  where $\tmop{NodePrefix}$, $\tmop{NodeType}$ and $\tmop{BigKeySize}$
  functions are defined in Definitions \ref{def-node-prefix},
  \ref{defn-nodetype} and \ref{defn-bigkeysize} respectively.
\end{definition}

\begin{definition}
  \label{defn-bigkeysize}We define $\tmop{BigKeySize}$ function for each node
  type of the Trie as follows: \
  \[ \begin{array}{lllll}
       \tmop{BigKeySize} (T) & : & \{ \tmop{Leaf}, \tmop{Extension} \} &
       \rightarrow & \mathbb{N}\\
...
\ref{sect-merkl-proof}, and computes the trie root hash.
...
\subsubsection{ext\_blake2\_256}
...
{\tmstrong{Arguments}}:
\begin{itemize}
  \item \tmverbatim{data}: a memory address pointing at the buffer and
