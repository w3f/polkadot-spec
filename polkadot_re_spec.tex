...
{\state{$(d, \pi) \leftarrow${\tmem{{\tmname{VRF}}}}($i, \tmop{sk}$)}}
...
based on the {\tmem{block time}} associated with blocks in the slot tail
defined in Definition \ref{defn-block-time}.

\begin{definition}
  \label{defn-block-time}The {\tmstrong{Block time}} of block $B$ for node $j$
  formally represented by {\tmstrong{$T^j_B$}} is the local time of node $j$
  when node $j$ has received the block $B$ for the first time. If the node $j$
  itself is the producer of $B$ or if the node $j$ was not connected to the
  network when block $B$ was produced then $T_B^j$ is set equal to the time
  that block is (claimed to be) produced. We may drop index $j$ in $T^j_B$
  notation if there is no ambiguity about the underlying node.
\end{definition}

\custombinding{5}{\noindent}\begin{tabular}{l}
  \hline
  \tmtextbf{Algorithm  5. } \label{algo-slot-time}{\tmname{Slot-Time}}($s$:
...
{\state{{\FORIN{$B_i$}{{\tmname{SubCkhain}}({\tmname{Last-Finalized-Slot}} $-
\tmop{SlTl}$, {\tmname{Last-Finalized-Slot}})}}}}

{\state{{\tmname{$s_t^{B_i} \leftarrow T^{}_{B_i}$+Slot-Offset($B_i,
s$)$\times \mathcal{T}_{}$}}}}
...
\custombinding{10}{\noindent}\begin{tabular}{l}
  \hline
  \tmtextbf{Algorithm  10. } {\tmname{Attempt-To-Finalize-Round}}($r$)\\
  \hline
\end{tabular}
