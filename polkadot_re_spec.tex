...
\begin{definition}
  We define the following complete order over $\mathbb{C}$ such that for $C_1,
  C_2 \in \mathbb{C}$ if $| C_1 | \neq | C_2 |$ we say $C_1 > C_2$ if and only
  if $| C_1 | > | C_2 |$.
...
Messages{\tmem{{\index{Transaction Message}}}} and keep a {\tmem{transaction
pool{\index{transaction pool}}}} and a {\tmem{transaction {\index{transaction
...
{\state{{\tmname{Add-To-Transaction-Queue($E, R$)}}}}

{\todo{Replace list of reaction based on R}}}}

\hrulefill{\medskip}

where {\todo{Add References for validate\_transaction, R, Longest-Chain,
Extrinsic-Queue}}
\begin{itemize}
  \ 
\end{itemize}
...
\custombinding{5}{\noindent}\begin{tabular}{l}
  \hline
  \tmtextsc{Algorithm  5. } {\tmname{Import-and-Validate-Block($B, \tmop{Just}
  (B)$)}}\\
  \hline
\end{tabular}
...
\custombinding{7}{\noindent}\begin{tabular}{l}
  \hline
  \tmtextsc{Algorithm  7. } \label{algo-slot-time}{\tmname{Slot-Time}}($s$:
...
\custombinding{14}{\noindent}\begin{tabular}{l}
  \hline
  \tmtextsc{Algorithm  14. } {\tmname{Attempt-To-Finalize-Round}}($r$)\\
  \hline
\end{tabular}
...
\subsection{\tmverbatim{TaggedTransactionQueue\_validate\_transaction}}\label{sect-validate-transaction}
