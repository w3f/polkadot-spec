...
ability of changing the runtime code.

The initial runtime code of the chain is embeded, as an extrinsics into the
chain initialization JSON file and is submitted to Polkadot RE (see Section
\ref{sect-genisis-block}).

Subsequent calls to the runtime has the ability of calling the storage API
...
the Runtime Wasm blob and has been exported as shown in Sinppet
...
{\tmfloat{h}{small}{table}{\begin{tabular}{l}
  ...
  \tmverbatim{(export "auythorities" (func \$authorities))}\\
...
Runtime API is set of functions which Polkadot RE exposes to Runtime in order
to access Storage content and other external functions. Some of the functions
are exposed to the runtime for efficiency reason. Here is the list of the
