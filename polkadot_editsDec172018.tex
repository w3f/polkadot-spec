...
\title{Polkadot Runtime Environment \\ {\Large Protocol Specification}}
...
\begin{definition}
  ...
  \begin{itemize}
    ...
    does not impose any limitation or specification for this field. Essentially,    it can be a byte array of any length. This field is indicated as
    {\tmstrong{$H_d$}}
  \end{itemize}
\end{definition}
...
to the Polkadot RE, for the block to be appended to the blockchain.
...
(see Section \ref{sect-runtime-api}) to insert a new Wasm blob into runtime
...
To authenticate the state of the system, the stored data needs to be re-arranged and hashed in a {\tmem{radix 16 tree}} also known as {\tmem{base-16 modified
Merkle Patricia Tree}}, which hereafter we will refer to as the
{\tmem{{\tmstrong{Trie}},}} in order to compute the hash of the whole state
storage consistently and efficiently at any given time.

As well, modification has been made in the storing of the the nodes' hash in the Merkle Tree structure to save space on entries storing small entries.
...
By looking at $k_{\tmop{enc}}$ as a sequence of nibbles, one can walk the radix
...
To validate that Extrinsics data are stored in a block across clients, Polkadot RE uses the same trie structure as for the state storage described in Section
...
Runtime API is a set of functions, which Polkadot RE exposes to Runtime in order
to access Storage content and other external functions. Some of the functions
