...
<<algo-encode-mortality>> and <<algo-decode-mortality>>.

[latexmath]
++++
\text{// If the extrinsic is immortal, specify}
\text{// a single byte with the valueequal to zero.}

\vcenter{\begin{flalign*}
  \Return & \begin{cases}
    0 & if\ extrinsic\ is\ immortal
  \end{cases}&
\end{flalign*}}

\textbf{Init} factor = \textsc{Limit}(M_{per} >> 12,\ 1,\ \phi)
\textbf{Init} left = \textsc{Limit}(\textsc{TZ}(M_{per})-1,\ 1,\ 15)
\textbf{Init} right = \frac{M_{pha}}{factor} << 4

\text{// Returns a two byte value}
\Return left|right
++++
