...
\For{$amount \leftarrow 1,MaxRegistrars$}
  ...
  \State $time \leftarrow$ \textsc{Timer}$($\textsc{Request-Judgement}$($\textsc{Random}$(amount), 100))$
  \State \textsc{Add-To}$(collection, time)$
\EndFor
\State $\mathcal{W} \leftarrow$ \textsc{Compute-Weight}$(collection)$
\Return $\mathcal{W}$
++++
...
\For{$amount \leftarrow 1,MaxNominatorRewardedPerValidator$}
  \For{$era\_depth \leftarrow 1,HistoryDepth$}
    ...
    \State $time \leftarrow$ \textsc{Timer}$($\textsc{Payout-Stakers}$(validator), era\_index)$
    \State \textsc{Add-To($collection$, $time$)}
  \EndFor
\EndFor
\State $\mathcal{W} \leftarrow$ \textsc{Compute-Weight}$(collection)$
...
\Function{Run-Benchmark}{$index$, $balance$}
  ...
  \State $recipient \leftarrow$ \textsc{Create-Accouny}$(\textit{"recipient"}, index)$
  \State \textsc{Set-Balance}$(sender, balance)$
  \State $time \leftarrow$ \textsc{Timer}$($\textsc{Transfer}$(sender, recipient, balance))$
  \Return $time$ 
\EndFunction
++++
...
\State \textbf{init} $collection = \{\}$
  \For{$balance \gets 1,100$}
    ...
    \State \textsc{Pass-Era}()
    \State $time \leftarrow$ \textsc{Timer}$($\textsc{Withdraw-Unbonded}$(controller))$
    \State \textsc{Add-To}$(collection, time)$
  \EndFor
  \State $\mathcal{W} \leftarrow$ \textsc{Compute-Weight}$(collection)$
  \Return $\mathcal{W}$
